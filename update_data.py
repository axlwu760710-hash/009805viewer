...
def run():
    ...
    html = html.replace("", f"{total_impact:+.2%}").replace("", f"{usd_change:+.2%}").replace("", f"{final_total:+.2%}")
    html = html.replace("", rows).replace("", datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
